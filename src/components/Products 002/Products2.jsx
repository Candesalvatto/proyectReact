...
import img from '../../assets/imgs/FleeceSuperCroppedHoodie.jpg'
import '../Products 001/Products.css'
import banner from '../../assets/imgs/DROP002.jpg'
...
export const Products2 = () => {
    ...
    useEffect(() => {
      ...
      getDocs(productsDB)
        .then((products) => {
          ...
          setFilteredProducts(allProducts.filter((product) => product.drop === 2));
        })
        .catch((error) => {
    ...
    useEffect(() => {
      setFilteredProducts(
        products.filter((product) => product.drop === 2 && product.name.toLowerCase().includes(form.lookFor.toLowerCase()))
      );
    ...
    const handleSubmit = (e) => {
      ...
      getDocs(q)
        .then((result) => {
          ...
          setFilteredProducts(foundProducts.filter((product) => product.drop === 2));
        })
        .catch((error) => {
          console.error('Error fetching products:', error);
        });
    ...
    return (
      <>
            ...
            <h1>DROP 002</h1>
            <h2>PUFFERS + BASE LAYERS</h2>
            <p>IN COLLABORATION WITH ENTIRE STUDIOS</p>
            </div>
            </div>
        </main>
